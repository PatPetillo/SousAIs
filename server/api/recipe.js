...
router.get('/searchRecipe/:info', (req, res, next) => {
  let { info } = req.params;
  info = info.split('-');
  const type = info[0];
  const cuisine = info[1];
  const diet = info[2];
  const exclude = info[3];
  const intolerances = info[4];
  return axios.get(`https://spoonacular-recipe-food-nutrition-v1.p.mashape.com/recipes/search?cuisine=${cuisine}&diet=${diet}&excludeIngredients=${exclude}&instructionsRequired=true&intolerances=${intolerances}&limitLicense=false&number=10&offset=0&query=${type}`, {
    headers: {
      'X-Mashape-Key': key,
